...
## v0.13.0 — Doc Map (`docmap`) (pending)
...
- [ ] **P0 — prescriptive structure index** — `docs/layout.md`, the
...
- [ ] **P1 — scan + data layer** — `internal/docmap` builds
      `.aikata/docmap.yaml` from frontmatter + link graph + filesystem tree;
      extract the shared `internal/docmeta` parser from `doctor` so link
      parsing cannot drift.
- [ ] **P2 — readable view** — `.aikata/docmap.md`: directory tree + Mermaid
      `doc → doc` graph (adjacency-list degrade past a node threshold) +
      `path → summary` index, with best-effort summary extraction.
- [ ] **P3 — `aikata map` + isolated hooks** — the explicit verb plus a
      final, isolated rebuild step in `init` / `fill` / `enable` / `sync` /
      `generate`, decoupled from per-tool `generate` provider failures.
- [ ] **P4 — `doctor` freshness check** — in-memory rebuild compared by
      `HashContent`; warning on drift, regenerated by `--fix`; `--json` code.
- [ ] **P5 — config + optional renderers** — `docmap.{formats,targets,exclude}`
