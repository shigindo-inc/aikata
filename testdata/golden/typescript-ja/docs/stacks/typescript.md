...
共有値の置き場所を先に決めておくことで、よくある AI 協業のドリフトを
防げる: 単一の真実源の合意が無いために、文字列・数値リテラルが各
モジュールに重複する、という失敗だ。推奨の置き場所（プロジェクトに
合わせて調整してよいが、1 つに決めて一貫させる）:

- `src/constants/` — 共有定数（キー、ルート、上限、フィーチャー
  フラグ）。2 箇所以上で使うリテラルは直書きせず、ここに置く。
- `src/config/` — ランタイム設定と環境変数の配線（一度だけ読み、
  型付けし、境界で検証する）。
- フロントエンドの場合: 色・余白・タイポグラフィの単一のデザイン
  トークン／テーマ置き場所（例: `src/theme/`）を設け、UI 値を
  コンポーネントごとに直書きしない。
...
_TODO: プロジェクトの形（バックエンドサービス vs フロントエンド
アプリ vs ライブラリ）に合わせてレイアウトを固定し、異なる場合は
ADR に記録する。デザイントークンの単一の真実源をパッケージが持つ場合は
リンクする。_
