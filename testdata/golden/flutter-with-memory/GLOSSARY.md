...
A handle to the location of a widget in the widget tree. Used to look
up inherited widgets (theme, navigator, providers, etc.). Holding a
`BuildContext` across an `await` boundary is unsafe — always check
`mounted` afterwards.
...
Dart's compile-time guarantee that non-nullable types cannot hold
`null`. Use `?` for nullable types and prefer `late` over `?` when the
non-nullness is invariant after initialization. Avoid `!` (force
non-null) without a comment justifying it.
...
The mechanism for sharing mutable state across the widget tree.
samplekata's chosen approach is captured in an ADR and explained
in [`docs/stacks/flutter.md`](./docs/stacks/flutter.md).
...
The composable building block of the Flutter UI. A widget is an
immutable description; the framework materializes it into mutable
`Element` and `RenderObject` instances. See
[`docs/stacks/flutter.md`](./docs/stacks/flutter.md) for the
project's widget-authoring rules.
