...
1. [`docs/stacks/flutter.md`](./docs/stacks/flutter.md) — スタック固有ルール
...
| 作業の種類 | まず読むファイル |
|---|---|
| 新規 widget／画面の実装 | `SPEC.md`, `docs/stacks/flutter.md` |
| 状態管理の追加・変更 | `ARCHITECTURE.md`, `docs/stacks/flutter.md` |
| バグ修正 | `docs/tasks/current.md`, `test/` 配下の関連テスト |
| 設計判断の記録 | `docs/adr/` に新規ファイル。書式は [ADR 0001](./docs/adr/0001-record-architecture-decisions.md) |
| 用語の更新 | `GLOSSARY.md` を更新後、旧表記を grep |
| 厄介な問題の調査 | まず `docs/troubleshooting.md` |

## 4. 厳守ルール

- **シークレットをコミットしない。** 代わりに [`.env.example`](./.env.example) のパターンを参照。
- **作業の開始時と終了時に [`docs/tasks/current.md`](./docs/tasks/current.md) を更新する。**
- **新規コードにはテストを追加する**: widget tests を `test/` 配下に
  `lib/` と 1:1 で配置。完了宣言前に `flutter test` を必ず実行。
- **コミット前に `flutter analyze` を走らせる**。警告 0 が基準。
- **null safety を尊重する** — `!` で非 null assertion を黙らせるには、
  必ず理由を述べたコメントを残す。
...
1. [`docs/troubleshooting.md`](./docs/troubleshooting.md) を確認 — 問題が既知である可能性が高い。
2. [`docs/adr/`](./docs/adr/) で過去の設計判断を確認。
3. [`docs/stacks/flutter.md`](./docs/stacks/flutter.md) でスタック固有のガイダンスを確認。
4. 質問を文書化し、`docs/tasks/current.md` に追記してメンテナーに surface する。
   アーキテクチャ上の重要点を黙って推測しない。
