...
hand-edit generated files (e.g. `CLAUDE.md`, `.cursor/rules/main.mdc`)
unless the user has explicitly opted out of regeneration.
...
- Wants to regenerate `CLAUDE.md` or `.cursor/rules/main.mdc` after
  `AGENTS.md` changes.
...
- Don't bump `updated:` manually on a stale doc; let `aikata doctor --fix`
  do it.
- Don't fabricate ADR numbers; `aikata` will own auto-numbering in v0.4
  via `aikata add adr`.
