...
func TestRunPrompt_HappyPath(t *testing.T) {
	...
	// changelog, monorepo, prompts, modeling, env.
	...
	prompts := out.String()
	for _, needle := range []string{"Project name", "Scope", "Stack", "Document language", "AI tools", "memory", "UI.md", "API.md", "testing.md", "CHANGELOG.md", "monorepo", "prompts.md", "usecases.md"} {
...
func TestRunPrompt_KeepsDefaultsOnBlankInput(t *testing.T) {
	// Blanks (scope + stack + lang + ai-tools + 4 v0.4 single-file +
	// 1 v0.6 monorepo + 1 v0.9.2 prompts + 1 modeling + 1 v0.9.7 env
	// question) accept every default.
	in := strings.NewReader("myproj\n" + strings.Repeat("\n", 13))
	var out bytes.Buffer
...
func TestRunPrompt_OptionalComponentSkipsHonored(t *testing.T) {
	...
	for _, needle := range []string{"memory", "UI.md", "API.md", "testing.md", "CHANGELOG.md", "monorepo", "prompts.md", "usecases.md"} {
