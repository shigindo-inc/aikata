// Package glob implements the small slash-path glob matcher shared by
// `aikata doctor` (`doctor.exclude`, ADR 0021) and `aikata sync`
// (`sync.own`, ADR 0025). It is intentionally minimal and dependency-
// free so both features apply identical matching semantics.
package glob
...
// slash-form relative to the project root. See ADR 0021 for the
// supported glob shape.
