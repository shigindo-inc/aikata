...
The CLI exposes the following commands. Each is described in terms of
**what** it must do and the user-visible behavior. Implementation details
...
**Purpose**: Regenerate the **doc map** — a derived, doc-cartography
artifact describing the document set itself (inventory, cross-references,
freshness, managed/external split). Decided by
[ADR 0044](./docs/adr/0044-doc-map-derived-artifact.md).

**Behaviour**:

- Scans the configured document surface (default: all Markdown, minus
  `.aikata/**` and generated AI-tool artifacts) **reading documents
  only** — no source code, no language-specific analysis, so the
  stack-agnostic core is preserved.
- From one scan, writes two mandatory machine-zone renderings:
  `.aikata/docmap.yaml` (structured data layer / single source of the
  map's truth) and `.aikata/docmap.md` (a directory tree + a Mermaid
  link-graph + one-line summaries). `txt` / `json` / `mmd` are optional
  config-gated renderers.
- Each document is tagged `managed` (in aikata's managed surface) or
  external; per-document summaries are extracted best-effort
  (`summary:` frontmatter → leading `>` blockquote → first paragraph →
  H1 → filename), so adopting aikata over an existing repo needs no
  document refactor.
- The same rebuild runs as a **final, isolated step** of `init`, `fill`,
  `enable`, `sync`, and `generate`, decoupled from per-tool `generate`
  provider failures (a failing provider must never leave the map stale,
  and vice versa).
- `docmap.*` are aikata-owned derived state: **not** manifest-tracked and
  **not** subject to `sync`'s 3-way merge. `aikata doctor` instead carries
  a freshness check (warning on drift, regenerated by `--fix`).
- The tracked set and output formats are configurable via the `docmap`
  section of `.aikata/aikata.yaml`.
...
- aikata does **not** read or parse source code — including the doc map
  (§4.9), which is built from documents only, never from an AST or
  dependency graph. The stack-agnostic core knows no language.
- aikata does **not** perform runtime context selection or token
  counting; the doc map is a scaffold-time artifact, not an inference-time
  context router.
